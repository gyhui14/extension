...
def get_cca_similarity(acts1, acts2, epsilon=0., threshold=0.98,
  ...
  '''
  o1 = o2 = acts1.shape[0]  # neuron

  m = acts1.shape[1] # batch size
  H1 = acts1
  H2 = acts2

  H1bar = H1 - np.mean(H1, axis=1, keepdims=True)
  H2bar = H2 - np.mean(H2, axis=1, keepdims=True)

  sigmaxy = (1.0 / (m - 1)) * np.dot(H1bar, H2bar.T) 
  sigmayx = (1.0 / (m - 1)) * np.dot(H2bar, H1bar.T) 
  sigmaxx = (1.0 / (m - 1)) * np.dot(H1bar, H1bar.T)
  sigmayy = (1.0 / (m - 1)) * np.dot(H2bar, H2bar.T )
  '''
...
results_t = get_cca_similarity(A_fake, At_fake)
